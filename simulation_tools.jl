...
# this function generates a realization of the noises on all the nodes in the graph
function noise_values(g::Vector{H}, x::Vector{F}) where F <: AbstractFloat where H <: Function
    return x .|> g
end

# this function plots some trajectories of the dynamics together with the average trajectory
function plot_trajectories(sim_res::simulation_data, Nsamples::Int64)
...
function stats_analysis(sim_res::simulation_data)
...
function plot_stats(μ::Vector{Float64}, C::Matrix{Float64}, ρ::Matrix{Float64}, t₀::Int64, tₙ::Int64)
    plot(t₀:tₙ, μ[t₀:tₙ], layout=(3,1), subplot=1, xlabel=L"T", ylabel=L"\mu(t)", size=(1000,800), color="green", margin=10mm, label=false)
    plot!(t₀:tₙ, C[t₀:tₙ,t₀], subplot=2, xlabel=L"T", ylabel=L"C(t-t_{0},t_{0})", color="red", label=false)
    plot!(t₀:tₙ, ρ[t₀:tₙ,t₀], subplot=3, xlabel=L"T", ylabel=L"\rho(t-t_{0},t_{0})", color="orange", label=false)
end

# this function saves the trajectories obtained from the simulation
function save_trajectories(sim_res::simulation_data, folder_path::String)
